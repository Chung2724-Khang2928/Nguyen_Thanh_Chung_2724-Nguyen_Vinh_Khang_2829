...
/* System call exit 
 * Kiem tra luong hien tai co ton tai tien trinh con(cp).
 * Cap nhat thong tin status cua tien trinh con ve -1 va cap nhat cho tien trinh cha.
 */
void
...
/* syscall exec
 * thuc thi tien 1 dong lenh command line va tra ve gia tri pid cua tien trinh duoc thuc thi
 */
pid_t
syscall_exec(const char* cmdline)
{
    ...
    /* check if process if loaded -> giam gia tri sema va cho giam ve gia tri duong*/
    if (child_process_ptr->load_status == NOT_LOADED)
    {
      sema_down(&child_process_ptr->load_sema);
    }
    /* check if process failed to load -> xoa tien trinh con ay di va tra ve loi*/
